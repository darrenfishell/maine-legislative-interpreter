...
class Config:
    ...
    RAW_SCHEMA: str = os.getenv('MLI_RAW_SCHEMA', 'raw')
    STAGING_SCHEMA: str = os.getenv('MLI_STAGING_SCHEMA', 'staging')
    INTERMEDIATE_SCHEMA: str = os.getenv('MLI_INTERMEDIATE_SCHEMA', 'intermediate')
    MART_SCHEMA: str = os.getenv('MLI_MART_SCHEMA', 'mart')

    # Text cleaning
    ENABLE_SPELL_CHECK: bool = os.getenv('MLI_ENABLE_SPELL_CHECK', '0') in ('1', 'true', 'True')
    ...
    # Paths — repo root is two levels above src/dlt_pipeline/
    PROJECT_ROOT: Path = Path(__file__).resolve().parents[2]
    DATA_DIR: Path = Path(os.getenv('MLI_DATA_DIR', PROJECT_ROOT / 'data'))
    PDF_DIR: Path = Path(os.getenv('MLI_PDF_DIR', DATA_DIR / 'testimony_pdfs'))
